...
pitchers_file <- "data/depthChartProjections_pitchers_20190323.csv"
batters_file <- "data/depthChartProjections_batters_20190323.csv"
...
# Draft Prep
The above plot is reproduced below to exclude known keepers and include pitchers. 
...
Below is a plot of z_score by round so I can identify in which rounds to target specific positions, particularly once we pass to below a z_score of 10. 
```{r echo=FALSE}
positions_ranked_with_keepers %>%
  filter(!keeper,
         ADP <=300) %>%
...
Here's the data set with which I'll print a little cheat sheet (or interactive app?) for the draft.

```{r echo=FALSE}
batters_ranked %>%
  select(Name, z_rank, z_sum, ADP, positions, display_position) -> batters_draft
  
batters_draft %>%
  mutate(rn = row_number()) %>% 
  filter(grepl("Robertson",Name)) %>%
  select(rn) %>%
  pull() -> robertson_drop

batters_draft %>%
  mutate(rn = row_number()) %>% 
  filter(grepl("Jose Ramirez",Name)) %>%
  select(rn) %>%
  pull() -> jramirez_drop  
  
rows_drop <- c(robertson_drop[-1],jramirez_drop[-1])

batters_draft %>%
  slice(-rows_drop) -> batters_draft

pitchers_ranked %>%
  select(Name, z_rank, z_sum, ADP, positions, display_position) -> pitchers_draft
  
pitchers_draft %>%
  mutate(rn = row_number()) %>%
  filter(grepl("Cody Reed",Name)) %>%
  select(rn) %>%
  pull() -> reed_drop

pitchers_draft %>%
  mutate(rn = row_number()) %>%
  filter(grepl("Will Smith",Name)) %>%
  select(rn) %>%
  pull() -> smith_drop

rows_drop <- c(smith_drop[-1], reed_drop[-2])

pitchers_draft %>%
  slice(-rows_drop) -> pitchers_draft

rm(rows_drop, robertson_drop, jramirez_drop, reed_drop, smith_drop)

pitchers_draft %>%
  bind_rows(batters_draft) %>% 
  mutate(round = ceiling(ADP/12),
         keeper = (Name %in% keepers)) %>% 
  arrange(z_sum %>% desc) %>%
  mutate(z_rank = row_number()) %>%
  select(z_rank, Name, z_sum, round, ADP, positions, display_position, keeper) -> draft_df

draft_df %>% saveRDS("data/draft_dataset_with_keepers")

draft_df %>% filter(!keeper) %>% saveRDS("data/draft_dataset.Rds")

draft_df %>%
  select(-display_position, -keeper) %>%
  head(25) %>%
  kable()
```

# Post-Draft Thoughts
Overall I'm happy with the rankings and interactive cheat sheet - both helped me make decisions during the draft to execute on my strategy that will hopefully return a good team this year. That said, there are some things I should revist next year, noted below. Over the course of the year, I should capture weekly performance so I can get a sense of variability. I should also review the end of year z_scores to see how my draft strategy, z_score computation, and other facets worked. 

### Improvements

##### Relative value between pitchers and batters
Even though my strategy was to value pitchers less than batters, I was about halfway through the draft where the bottom of the barrel batters were clearly overvalued compared to remaining pitchers. As a result I probably picked too many batters rather than filtering batters out entirely. By end of year, I should have a good sense of what a "replacement" player value is, and reframe the z_scores relative to that. 

##### Reductive power of z_scores
It's possible I draft a number of high z_score players whose category makeup is very similar, leading to a smaller number of categories in which I'm competitive than I want. 