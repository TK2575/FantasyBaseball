...
proj_batter <- add_z_scores_to_projections("data/depthChartProjections_ros_batters_20190429.csv","batter") 
proj_pitcher <- add_z_scores_to_projections("data/depthChartProjections_ros_pitchers_20190429.csv", "pitcher")
...
rosters_with_adj_projections %>% 
  filter(grepl("Plouffe|Hebrew",team)) %>% 
  select(team, manager, name_full, display_position, zar) %>% 
  filter(grepl("deGrom|Rizzo",name_full)) %>% 
  group_by(team) %>% 
