...
if (force_all && (batter_file %>% is.null() || pitcher_file %>% is.null())) {
  stop("Need projection files to force a recalc")
} else {
  rosters <- load_players_with_projections(force, 
                                           force_all, 
                                           batter_file, 
                                           pitcher_file)
}
...
rosters %>% 
  filter(grepl("Plouffe|Matter",team)) %>% 
...
rosters %>% 
  ...
  filter(grepl("Hicks|Muncy",name_full)) 
# %>%
#   group_by(team) %>%
...
# Averaging pos_zar by position, overall
# Does this actually indicate good/bad teams?
rosters_by_position %>% 
  filter(team != "Free Agent") %>% 
...
team_pos_avg_zar %>% 
  ungroup() %>% 
  group_by(team) %>% 
  summarize(avg_pos_zar = mean(avg_pos_zar)) %>% 
  arrange(avg_pos_zar %>% desc)
```