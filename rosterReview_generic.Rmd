...
# TODO get position factor ordering back, summarize pitching to single point/range, add bench single point/range, consider DL?
rosters_by_position %>% 
...
team_pitchers_RP <- team_pitchers_all %>% 
  ...
  filter(rnk <= 6) %>% 
  select(position, player_id, name_full, team, manager, zar, pos_zar)

team_pitchers_SP %>% 
  bind_rows(team_pitchers_RP) %>% 
  group_by(team, position) %>% 
  summarize(zar = mean(zar),
            pos_zar = mean(pos_zar)) -> team_pitchers_starters
...
team_batters_starters %>% 
  bind_rows(team_batters_bench) %>% 
  bind_rows(team_pitchers_starters)

#FIXME team_batters_starters needs MANAGER column
...
team_pitchers %>% 
  bind_rows(team_batters_starters) %>% 
  ggplot(aes(position, pos_zar, color = position, alpha = zar)) +
  geom_point() +
