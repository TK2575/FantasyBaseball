...
rosters %>% 
  filter(grepl("Plouffe|Dorn",team)) %>% 
...
rosters %>% 
  filter(grepl("Dorn|process",team)) %>% 
  select(team, manager, name_full, display_position, zar) %>% 
  filter(grepl("Trout|Whitley|Cain|Price",name_full)) %>% 
  group_by(team) %>% 
  summarize(zar = sum(zar))

rosters %>% 
  filter(grepl("Dorn|process",team)) %>% 
  select(team, manager, name_full, display_position, war) %>% 
  filter(grepl("Trout|Whitley|Cain|Price",name_full))
```
